 ...
 btnpost.addEventListener("click", ()=>{
    ...
    .then (json=>{ 
        ...
        json.forEach(pub=>{
            res+=`<h3>${pub.title}</h3> 
                  <p>${pub.body}</p>
                  <button type="button" onClick="cargarcoments(${pub.id})">ver coemtarios</button>
  ...
  function cargarcoments(blog){
      ...
      .then(json=>{
          let res1="";
          json.forEach(pub=>{
              estructura.set(pub.id,pub.email,pub.body);
            res1+=`
                  <p>${pub.email}</p>
                  <p>${pub.body}</p>
                  <div id="blog${pub.id}"></div>`
          })
          document.getElementById("blog"+blog).innerHTML="<h4>Comentario</h4>"+res1;
      })
  }